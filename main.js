...
class FilterConditions{
    ...
    this.shownTypes = {'Planet': true, 'Dwarf planet':false, 'NEO':false, 'Shower':false}
    }
...
function addSun() {
    ...
    const geometry = new THREE.SphereGeometry(0.02*10, DEFAULT_MESH_N, DEFAULT_MESH_N);
...
async function initializePlanets() {
    ...
    for (const [planetName, planetData] of Object.entries(planets_json)) {
        ...
        if (planetName === 'rings'){
            const geometry = new THREE.RingGeometry(planetData.renderParams.innerRadius*10, planetData.renderParams.outerRadius*10, 64);
        ...
        // if not ring do sphere
            ...
            const geometry = new THREE.SphereGeometry(planetData.renderParams.radius*10, DEFAULT_MESH_N, DEFAULT_MESH_N);
...
// Removed createRadialGradientPlane and all related code
...
function updateOrbits(filterConditions) {
    ...
    // Removed radialGradientPlane removal
    ...
    // Sporadics
    // Removed Sporadic meteoroid complex functionality
...
// --- Pop-out image overlay logic ---
function showPlanetImageOverlay(imgSrc) {
    let overlay = document.getElementById('planet-image-overlay');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'planet-image-overlay';
        ...
        overlay.innerHTML = `
            <div style="position:relative; background:#222; border-radius:18px; box-shadow:0 2px 24px #000; padding:24px;">
                <img id="planet-popup-img" src="${imgSrc}" alt="Planet" style="max-width:480px; max-height:480px; border-radius:16px; box-shadow:0 0 32px #fff;">
                <button id="planet-popup-close" style="position:absolute; top:8px; right:8px; background:#444; color:#fff; border:none; border-radius:8px; font-size:1.2rem; padding:6px 16px; cursor:pointer;">Close</button>
            </div>
        `;
        document.body.appendChild(overlay);
        document.getElementById('planet-popup-close').onclick = hidePlanetImageOverlay;
    } else {
        document.getElementById('planet-popup-img').src = imgSrc;
        overlay.style.display = 'flex';
    }
}
function hidePlanetImageOverlay() {
    const overlay = document.getElementById('planet-image-overlay');
    if (overlay) overlay.style.display = 'none';
}

// Function to draw a planet
function drawPlanet(planet, isSelected) {
    ctx.save();
    if (isSelected) {
        // Pop-out effect: increase size and add glow
        ctx.shadowColor = '#ffd700';
        ctx.shadowBlur = 24;
        ctx.beginPath();
        ctx.arc(planet.x, planet.y, planet.radius*8, 0, 2 * Math.PI);
